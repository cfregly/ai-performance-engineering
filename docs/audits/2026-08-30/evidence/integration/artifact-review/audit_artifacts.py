"""Read frozen audit receipts; write only this new integration review directory."""
...
SUFFIXES = {".json", ".jsonl", ".txt", ".log", ".xml", ".md", ".py", ".cu", ".cuh", ".hpp", ".h", ".sh", ".toml", ".html", ".yaml", ".yml", ".cpp", ".cmake", ".csv", ".patch"}
...
def main():
    ...
    files = [path for path in EVIDENCE.rglob("*") if path.is_file() and HERE not in path.parents]
    ...
    extra_records = {"report.json", "original-sources.json", "source-changes.json", "log-migration.json"}
    manifests = [path for path in files if path.suffix == ".json" and
                 (any(part in path.name.lower() for part in ("receipt", "manifest", "source-files")) or
    ...
    for ref in refs:
        if (not ref["resolved_path"].startswith(("docs/audits/", "/")) and
    ...
    artifact_mismatches = [ref for ref in refs if ref["resolved_path"].startswith("docs/audits/") and
    ...
    (HERE / "all-hash-references.json").write_text(json.dumps(refs, indent=2) + "\n")
