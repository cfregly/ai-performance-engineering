...
- Historical direct-runtime claims for grouped backward and layer do not establish current acceptance. The old layer forward tolerance accepted all-zero output on bounded real CPU fixtures, and its32x32payload omitted the last rows/channels. Current layer forward requires full-output reference checks and a reviewed accuracy policy; CUDA/runtime/performance gates remain pending.
