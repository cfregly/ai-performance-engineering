...
class DeepSeekHybridEPModule(nn.Module):
    ...
    def _roundtrip_routes(
        ...
        *,
        ...
        if use_single:
            recv_tokens = self._all_to_all_single(sorted_tokens, send_counts, recv_counts, group=group, label=f"{event_label}.recv_tokens", reuse=reuse)
            recv_weights = self._all_to_all_single(sorted_weights, send_counts, recv_counts, group=group, label=f"{event_label}.recv_weights", reuse=reuse)
