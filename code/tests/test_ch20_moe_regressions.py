...
from ch20.optimized_moe import ToyMoe


REPO_ROOT = Path(__file__).resolve().parents[1]


def test_optimized_moe_routes_without_softmax_or_float_masks() -> None:
    source = (REPO_ROOT / "ch20" / "optimized_moe.py").read_text(encoding="utf-8")
    forward_section = source.split("def forward(self, x: torch.Tensor)", maxsplit=1)[1].split(
        "class OptimizedMoeBenchmark",
        maxsplit=1,
    )[0]
    ...
    model = ToyMoe(hidden_dim=8).eval()
