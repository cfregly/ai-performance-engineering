...
def test_ch02_cublas_benchmarks_reuse_output_buffer() -> None:
    for name in ("baseline_cublas.py", "optimized_cublas.py"):
...
def test_ch09_optimized_memory_bound_reuses_compiled_output_buffer() -> None:
    source = (REPO_ROOT / "ch09" / "optimized_memory_bound.py").read_text(
        encoding="utf-8"
    )
    setup_section = source.split("def setup", maxsplit=1)[1].split(
        "def benchmark_fn",
...
def test_ch06_optimized_add_reuses_output_buffer() -> None:
    source = (REPO_ROOT / "ch06" / "optimized_add.py").read_text(encoding="utf-8")
    setup_section = source.split("def setup", maxsplit=1)[1].split(
        "def benchmark_fn",
        maxsplit=1,
    )[0]
...
def test_ch04_optimized_nccl_reduction_buffers_skip_setup_zero_fill() -> None:
    ...
    assert "self._model_shard_view: Optional[torch.Tensor] = None" in source
    assert (
...
def test_ch01_fp16_and_ch04_nvls_cache_nvtx_enablement() -> None:
    ch01_source = (REPO_ROOT / "ch01" / "baseline_performance_fp16.py").read_text(encoding="utf-8")
    nvls_source = (REPO_ROOT / "ch04" / "nvls_collectives.py").read_text(encoding="utf-8")
    ch01_setup = ch01_source.split("def setup", maxsplit=1)[1].split(
        "def benchmark_fn",
        maxsplit=1,
    )[0]
    ch01_benchmark = ch01_source.split("def benchmark_fn", maxsplit=1)[1].split(
        "def get_benchmark",
        maxsplit=1,
    )[0]
    nvls_setup = nvls_source.split("def setup", maxsplit=1)[1].split(
        "def benchmark_fn",
        maxsplit=1,
    )[0]
    nvls_benchmark = nvls_source.split("def benchmark_fn", maxsplit=1)[1].split(
        "def capture_verification_payload",
        maxsplit=1,
    )[0]

    for setup_section in (ch01_setup, nvls_setup):
        assert "self._enable_nvtx = get_nvtx_enabled(config) if config else False" in setup_section

    for benchmark_section in (ch01_benchmark, nvls_benchmark):
        assert "get_config()" not in benchmark_section
        assert "get_nvtx_enabled(" not in benchmark_section
        assert "enable=self._enable_nvtx" in benchmark_section
...
def test_ch10_attention_reuses_verification_buffers() -> None:
    for name in ("baseline_attention.py", "optimized_attention.py"):
        ...
        assert "self._verify_output_buffer: Optional[torch.Tensor] = None" in source
        assert "self._verify_output_buffer = torch.empty_like(self.query, dtype=torch.float32)" in setup_section
        assert "self._verify_output_buffer.copy_(self.output)" in capture_section
        assert "output=self._verify_output_buffer" in capture_section
        assert "self.output.detach().float().clone()" not in capture_section
        assert "self._verify_output_buffer = None" in teardown_section
...
def test_ch19_dynamic_precision_batches_confidence_metric_reads() -> None:
    ...
    assert "confidence_stats = torch.empty(2, device=logits.device, dtype=torch.float32)" in decision_section
    assert "confidence_stats[0].copy_(entropy_values.mean())" in decision_section
    assert "confidence_stats[1].copy_(probs.max(dim=-1).values.mean())" in decision_section
    assert "confidence_stats_host = confidence_stats.detach().cpu()" in decision_section
    assert "entropy = float(confidence_stats_host[0])" in decision_section
    assert "max_prob = float(confidence_stats_host[1])" in decision_section
    assert "confidence_stats = torch.stack(" not in decision_section
    ...
    assert "entropy_stats_host = entropy_stats.detach().cpu()" in demo_entropy_section
    assert "high_entropy = float(entropy_stats_host[0])" in demo_entropy_section
    assert "low_entropy = float(entropy_stats_host[1])" in demo_entropy_section
    assert "entropy_stats = torch.stack(" not in demo_entropy_section
...
def test_moe_cuda_grouped_router_reuses_static_dispatch_buffers() -> None:
    ...
    assert "flat_tokens = tokens.index_select(0, token_indices)" not in base_forward_section
    ...
    assert "flat_tokens = self._flat_tokens_for(tokens, token_indices)" in forward_section
    assert "flat_tokens = tokens.index_select(0, token_indices)" not in forward_section
    ...
    flat_tokens_ptr = model._flat_tokens_buffer.data_ptr()
    ...
    assert model._flat_tokens_buffer.data_ptr() == flat_tokens_ptr
    torch.testing.assert_close(
...
def test_ch18_speculative_decoder_batches_match_control_reads() -> None:
    ...
    assert "self._all_matches_workspace" not in decoder_section
    ...
    assert "match_summary = self._match_count_workspace(matches.device)" in decode_section
    assert "torch.sum(matches, dim=None, out=match_summary)" in decode_section
    assert "torch.all(matches" not in decode_section
...
def test_ch04_baseline_nvlink_multigpu_reuses_copy_groups() -> None:
    for relative in (
        "ch04/baseline_nvlink_multigpu.py",
        "ch04/baseline_nvlink_topology_aware_multigpu.py",
    ):
        source = (REPO_ROOT / relative).read_text(encoding="utf-8")
        setup_section = source.split("def setup", maxsplit=1)[1].split(
            "def benchmark_fn",
        ...
        assert "self._copy_groups: List[Tuple[torch.Tensor, torch.Tensor, torch.Tensor]] = []" in source
        assert "self._copy_groups = list(zip(self.host_buffers, self.src, self.dst, strict=True))" in setup_section
        assert "for host_buffer, src, dst in self._copy_groups:" in benchmark_section
        assert "zip(self.host_buffers, self.src, self.dst" not in benchmark_section
        assert "self._copy_groups = []" in teardown_section
...
def test_ch05_distributed_reduction_defers_verification_scalars_outside_hot_loop() -> None:
    ...
    assert "self.local_sums = [torch.empty(1, device=t.device, dtype=torch.float32) for t in self.data]" in baseline_setup
    ...
    assert "self._host_total.add_(self.host_sums[idx])" in baseline_benchmark
...
def test_nanochat_chat_sft_batches_training_log_syncs() -> None:
    ...
    assert "losses = []" not in eval_section
    assert "losses.append(loss)" not in eval_section
    assert "torch.stack(losses)" not in eval_section
    assert "eval_loss_buffer = torch.empty(eval_steps, dtype=torch.float64, device=device)" in source
    assert "for eval_idx in range(eval_steps):" in eval_section
    assert "eval_loss_buffer[eval_idx].copy_(loss.detach())" in eval_section
...
def test_ch16_demo_causal_lm_reuses_kv_stack_buffers() -> None:
    ...
    assert "self._local_value_slots = [hidden] * layer_count" in forward_section
    assert "local_keys = self._local_key_slots" in forward_section
    assert "local_values = self._local_value_slots" in forward_section
    assert "local_keys[layer_idx] = key_local" in forward_section
    assert "local_values[layer_idx] = value_local" in forward_section
    ...
    assert "local_keys: List[torch.Tensor] = [hidden] * layer_count" not in forward_section
    assert "local_values: List[torch.Tensor] = [hidden] * layer_count" not in forward_section
    assert "local_keys.append(" not in forward_section
...
def test_ch04_multi_node_bandwidth_uses_tensor_reduce_scatter() -> None:
    source = (REPO_ROOT / "ch04" / "multi_node_blackwell.py").read_text(
        encoding="utf-8"
    )
...
def test_ch13_context_parallel_all_gather_reuses_full_kv_buffers() -> None:
    ...
    assert "_CAUSAL_MASK_CACHE" in source
    assert "_RING_POSITION_VIEW_CACHE" in source
    assert "_RING_CAUSAL_MASK_CACHE" in source
    assert "def _causal_mask_for" in source
    assert "def _ring_causal_mask_for" in source
    assert "mask = _causal_mask_for(rank, seq_shard, world_size, scores.device)" in mask_section
    assert "return scores.masked_fill_(mask, float(\"-inf\"))" in mask_section
    assert "global_q, global_k = _causal_mask_position_views(rank, seq_shard, world_size, device)" in mask_section
    assert "return scores.masked_fill(global_k > global_q, float(\"-inf\"))" not in mask_section
    ...
    cp_common._CAUSAL_MASK_CACHE.clear()
    cp_common._RING_POSITION_VIEW_CACHE.clear()
    cp_common._RING_CAUSAL_MASK_CACHE.clear()
    ...
    mask = cp_common._causal_mask_for(1, 4, 2, torch.device("cpu"))
    mask_again = cp_common._causal_mask_for(1, 4, 2, torch.device("cpu"))
    ...
    ring_mask = cp_common._ring_causal_mask_for(1, 0, 4, torch.device("cpu"))
    ring_mask_again = cp_common._ring_causal_mask_for(1, 0, 4, torch.device("cpu"))

    assert mask_q_again.data_ptr() == mask_q.data_ptr()
    assert mask_k_again.data_ptr() == mask_k.data_ptr()
    assert mask_again.data_ptr() == mask.data_ptr()
    torch.testing.assert_close(mask, mask_k > mask_q)
    assert ring_q_again.data_ptr() == ring_q.data_ptr()
    assert ring_k_again.data_ptr() == ring_k.data_ptr()
    assert ring_mask_again.data_ptr() == ring_mask.data_ptr()
    torch.testing.assert_close(ring_mask, ring_k > ring_q)
...
def test_fp8_demo_and_moe_lab_defer_verification_clones_outside_hot_loop() -> None:
    ...
    assert "error_totals_host = error_sums.detach().cpu()" in perchannel_accuracy
    assert "pt_error_total = float(error_totals_host[0])" in perchannel_accuracy
    assert "pc_error_total = float(error_totals_host[1])" in perchannel_accuracy
    assert "error_sums.detach().cpu().tolist()" not in perchannel_accuracy
...
def test_ch13_static_fp8_wrappers_sample_verification_outputs() -> None:
    for filename in ("baseline_fp8_static.py", "optimized_fp8_static.py", "fp8_static_demo.py"):
        source = (REPO_ROOT / "ch13" / filename).read_text(encoding="utf-8")
        setup_section = source.split("def setup", maxsplit=1)[1].split(
            "def benchmark_fn",
            maxsplit=1,
...
def test_medusa_eagle_avoids_inner_loop_wall_clock_timing() -> None:
    ...
    assert "head_offsets = self._draft_head_offset_views[view_idx]" in seed_section
...
def test_ch04_nvshmem_microbench_defers_output_tensor_outside_hot_loop() -> None:
    ...
    assert "self._last_output_tensor: Optional[torch.Tensor] = None" in init_section
    assert "self._payload_input_tensors: Optional[Dict[str, torch.Tensor]] = None" in init_section
    ...
    assert "self._last_output_tensor[0] = self._last_output_values[0]" in capture_section
    assert "self._last_output = self._last_output_tensor" in capture_section
    assert "inputs=self._payload_input_tensors" in capture_section
    assert "torch.tensor(" not in capture_section
...
def test_ch15_wide_ep_packs_directly_into_reusable_buffers() -> None:
    ...
    assert "self._rank_copy_groups = [" in baseline_setup
    assert "zip(self._rank_indices, self._rank_offsets, strict=True)" in baseline_setup
    ...
    assert "for indices, recv_view in self._rank_copy_groups:" in baseline_benchmark
    assert "torch.index_select(flat, 0, indices, out=recv_view)" in baseline_benchmark
    assert "zip(self._rank_indices, self._rank_offsets" not in baseline_benchmark
    assert "recv_buf[start:end]" not in baseline_benchmark
...
def test_ch19_nvfp4_training_defers_verification_forward_outside_hot_loop() -> None:
    for filename in ("baseline_nvfp4_training.py", "optimized_nvfp4_training.py"):
        source = (REPO_ROOT / "ch19" / filename).read_text(encoding="utf-8")
        setup_section = source.split("def setup", maxsplit=1)[1].split(
            "def _calibration_warmup" if filename.startswith("optimized") else "def _train_step",
            maxsplit=1,
        )[0]
        benchmark_section = source.split("def benchmark_fn", maxsplit=1)[1].split(
            "def capture_verification_payload",
...
def test_ch13_fp4_perchannel_wrappers_sample_verification_outputs() -> None:
    for filename in ("baseline_fp4_perchannel.py", "optimized_fp4_perchannel.py"):
        source = (REPO_ROOT / "ch13" / filename).read_text(encoding="utf-8")
        setup_section = source.split("def setup", maxsplit=1)[1].split(
            "def benchmark_fn",
            maxsplit=1,
...
def test_ch13_memory_profiling_pair_keeps_compute_dtype_fixed_and_direct_output_capture() -> None:
    ...
    for source, setup, capture, teardown in (
        ...
        assert "self._verify_output_buffer.copy_(self.output)" in capture
        assert "output=self._verify_output_buffer" in capture
        assert "output=self.output.detach().float().clone()" not in capture
        assert "self._verify_output_buffer = None" in teardown
