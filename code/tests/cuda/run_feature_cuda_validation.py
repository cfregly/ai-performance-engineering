"""Compile production scalar/cluster controls and run full-output + sanitizer acceptance.
...
CASES = (("scalar_controls", 28), ("cluster_fp8_storage", 21))
...
def main() -> int:
    ...
    parser.add_argument("--arch", required=True, choices=("sm_90", "sm_100a", "sm_103a", "sm_120", "sm_121"))
    parser.add_argument("--output-dir", required=True, type=Path)
    parser.add_argument("--sanitizers", choices=("all", "none"), default="all")
    args = parser.parse_args()
    args.output_dir.mkdir(parents=True, exist_ok=False)
    source = Path(__file__).with_name("feature_cuda_validation.cu").resolve()
    ...
    for case, (name, count) in enumerate(CASES):
        ...
        command = [nvcc, "-O2", "-std=c++17", "-rdc=true", f"-arch={args.arch}",
                   f"-DFEATURE_VALIDATION_CASE={case}", f"-DFEATURE_VALIDATION_TARGET={target}",
        ...
        report.setdefault("binary_sha256", {})[name] = hashlib.sha256(binary.read_bytes()).hexdigest()
        marker = f"FEATURE_CUDA_PASS case={case} checks={count} full_output_and_canaries=checked"
    ...
    return finish(status, 0, f"{sum(count for _, count in CASES)} actual full-output/canary checks; no performance qualification")
