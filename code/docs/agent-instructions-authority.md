...
- Agents may terminate `ncu`/`nsys` processes when they deem it necessary within the authorized task, without requesting further confirmation. Confirm ownership and scope termination to the affected run, preferably through its scheduler job. Preserve existing results and profiler artifacts; log the action, reason, timestamp, and job/process identifiers in the queue log. Mark interrupted captures as incomplete and rerun any required validation before claiming completion.
