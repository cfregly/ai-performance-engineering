...
class BaselineDynamicRouterVllmBenchmark(VerificationPayloadMixin, BaseBenchmark):
    ...
    def _configure_cli_args(self, cli_args) -> None:
        ...
        self._prompt_lengths = vllm_runner.routing_prompt_lengths(cli_args)
        request_count = len(self._prompt_lengths)
        self.register_workload_metadata(
            requests_per_iteration=float(request_count),
            tokens_per_iteration=float(
                sum(self._prompt_lengths) + request_count * cli_args.max_tokens
            ),
        )
    ...
    def setup(self) -> None:
        ...
        self._mode_input = scalar_int_buffer(self, "_mode_input", 0)
        ...
        self._engine_session = vllm_runner.create_vllm_routing_session(
            "baseline",
    ...
    def benchmark_fn(self) -> None:
        if self._mode_input is None or int(self._mode_input[0]) != 0:
            raise RuntimeError("setup() must initialize baseline routing mode")
    ...
    def capture_verification_payload(self) -> None:
        ...
        self._set_verification_payload(
            inputs={
                "prompt_token_ids": self._prompt_token_ids,
                "mode": scalar_int_buffer(self, "_mode_input", 0),  # baseline
            },
            output=self.output,
            batch_size=1,
            parameter_count=0,
