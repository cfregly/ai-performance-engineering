...
// Adapted from fast.cu, Copyright (c) 2024 Pranjal Shankhdhar.
// SM100 K64 port of the rung-5 two-CTA NVFP4 GEMM.
#define NVFP4_BUILD_NAME "sm100-r5-k64"
// Read-only metadata for main.cu. These do not select code here.
...
// address. nvcc 13.1 folds it and emits identical SASS without it.
...
// 3D tiled load with cta_group::2 multicast. The mask is the
...
// The 512-byte SF tiles form a 4D box. The w coordinate selects the slot.
...
// Legacy operand descriptor for a K64 slice in a 128-byte-swizzled window.
// K64 requires bit 52 clear and even 16-byte address units.
__host__ __device__ static __forceinline__ constexpr uint64_t
mma_smem_desc_k64_sw128(uint32_t window_base_1024aligned, int intra_window_off = 0) {
    uint64_t addr_field = uint64_t(((window_base_1024aligned >> 4) & 0x7FC0u)
                                   + uint32_t(intra_window_off >> 4));
    return addr_field | 0x4000404000000000ull;
}
...
// ============================================================================
// 256x256 output per 2-CTA cluster with eight K64 MMAs per K group.
// ============================================================================
namespace nvfp4 {
...
constexpr int CD_SFA_TILES  = 2;
constexpr int CD_SFB_TILES  = 4;
constexpr int CD_SFA_STRIDE = CD_SFA_TILES * CD_SF_TILE;   // 1024
constexpr int CD_SFB_STRIDE = CD_SFB_TILES * CD_SF_TILE;   // 2048
...
constexpr uint32_t SF_TX = uint32_t(CD_SFA_STRIDE + CD_SFB_STRIDE) *
        uint32_t(CTA_GROUP);                                           // 6144
static_assert(AB_TX == 65536u && SF_TX == 6144u,
              "expect_tx derivation drifted");

// The map IS the struct: member sizes and alignment produce every byte
// offset. The mainloop reads raw-address constants with
...
static_assert(CD_AB_SLOTS != 6 ||
              (CD_B_OFF == 99328 && CD_SFA_OFF == 197632 &&
               CD_SFB_OFF == 204800 && CD_TOTAL == 219136),
...
//
// Each 512-element K group uses two A/B windows and four scale slots.
// ============================================================================
namespace nvfp4 {
...
// One K64 NVFP4 MMA. A zero acc_pred overwrites the accumulator.
...
// One SF ring slot to TMEM. SFB order interleaves the two N blocks.
static __device__ __forceinline__ void cp_sf_buffer(
        uint32_t el, uint32_t slot,
        uint32_t sfa0, uint32_t sfa1,
        uint32_t sfb0, uint32_t sfb1, uint32_t sfb2, uint32_t sfb3,
        uint32_t sfa_hi, uint32_t sfa_lo, uint32_t sfb_hi, uint32_t sfb_lo) {
    constexpr uint32_t SFA_SLOT_U = CD_SFA_STRIDE / 16u;   // 64
    constexpr uint32_t SFB_SLOT_U = CD_SFB_STRIDE / 16u;   // 128
    ...
    cp_sf(el, sfb1, b + 2u*TILE_U,  sfb_hi);
    cp_sf(el, sfb2, b + 1u*TILE_U,  sfb_hi);
    cp_sf(el, sfb3, b + 3u*TILE_U,  sfb_hi);
}

// Operand descriptor low word for one of four K64 slices in a ring slot.
static __device__ __forceinline__ uint32_t ab_desc_lo(
        uint32_t smbase, uint32_t ab_base_off, uint32_t slot, uint32_t step) {
    const uint64_t d = ptx::mma_smem_desc_k64_sw128(
            smbase + ab_base_off + slot * CD_AB_STRIDE, int(step) * 32);
    return uint32_t(d & 0xFFFFFFFFu);
}


// Advance one producer-consumer ring cursor.
template <uint32_t Slots>
static __device__ __forceinline__ void advance_ring(
        uint32_t& slot, uint32_t& phase) {
    const uint32_t next = slot + 1u;
    const bool wrap = next == Slots;
    slot = wrap ? 0u : next;
    phase ^= wrap ? 1u : 0u;
}

// One output tile with eight K64 MMAs per 512-element K group.
static __device__ __forceinline__ MainloopState mainloop_tile(
        uint32_t taddr, uint32_t smbase, MainloopState in,
        uint16_t commit_mask, uint32_t groups) {
    ...
    uint32_t ab_phase = in.ab_phase;
    uint32_t ab_slot = in.ab_slot;
    uint32_t sf_phase = in.sf_phase;
    uint32_t sf_slot = in.sf_slot;
    constexpr uint32_t el = 1u;

    constexpr uint32_t SF = uint32_t(SF_TMEM_BASE);
    const uint32_t sfa_t0 = taddr + SF + 0u;
    const uint32_t sfa_t1 = taddr + SF + 4u;
    const uint32_t sfb_t0 = taddr + SF + 8u;
    const uint32_t sfb_t1 = taddr + SF + 12u;
    const uint32_t sfb_t2 = taddr + SF + 16u;
    const uint32_t sfb_t3 = taddr + SF + 20u;

    const uint64_t sfb_src =
            ptx::mma_sf_cp_src_desc_cd(smbase + uint32_t(CD_SFB_OFF));
    const uint32_t sfb_src_lo = uint32_t(sfb_src);
    const uint32_t sfb_src_hi = uint32_t(sfb_src >> 32);
    const uint64_t sfa_src =
            ptx::mma_sf_cp_src_desc_cd(smbase + uint32_t(CD_SFA_OFF));
    const uint32_t sfa_src_lo = uint32_t(sfa_src);
    const uint32_t sfa_src_hi = uint32_t(sfa_src >> 32);

    const uint32_t a_desc_hi =
            uint32_t(ptx::mma_smem_desc_k64_sw128(smbase + A_OFF) >> 32);
    const uint32_t b_desc_hi =
            uint32_t(ptx::mma_smem_desc_k64_sw128(smbase + B_OFF) >> 32);
    static_assert((0x4000404000000000ull & (1ull << 52)) == 0,
                  "K64 operand descriptor must use legacy LBO mode");

    constexpr uint32_t IDESC = ptx::mma_inst_desc_mxf4nvf4_block16(
            256u, 256u, 0u, 0u, ptx::Major::K, ptx::Major::K,
            false, false, false, ptx::ScaleFormat::E4M3);
    static_assert(IDESC == 0x10400480u, "K64 NVFP4 idesc drifted");
    const uint32_t idesc_s0 =
            ((sfa_t0 >> 1) & 0x60000000u) |
            ((sfb_t0 >> 26) & 48u) | IDESC;
    const uint32_t idesc_s1 =
            ((sfa_t1 >> 1) & 0x60000000u) |
            ((sfb_t2 >> 26) & 48u) | IDESC;

    const uint32_t acc_wait_parity = d_parity;
    d_parity ^= 1u;
    const uint32_t d_tmem = d_parity * uint32_t(D_STRIDE) + taddr;
    uint32_t acc_pred = 0u;
    bool need_acc_wait = true;

    #pragma unroll 1
    for (uint32_t g = 0; g < groups; ++g) {
        const uint32_t sf0 = sf_slot;
        const uint32_t sf_bar0 = smbase + (sf0 << 3) + SF_READY_OFF;
        mbar_wait_relaxed(sf_bar0, sf_phase);
        cp_sf_buffer(el, sf0, sfa_t0, sfa_t1,
                     sfb_t0, sfb_t1, sfb_t2, sfb_t3,
                     sfa_src_hi, sfa_src_lo, sfb_src_hi, sfb_src_lo);
        commit_consumed(el, smbase,
                        (sf_bar0 + SF_FREE_DELTA) - smbase, commit_mask);
        advance_ring<SF_RING_SLOTS>(sf_slot, sf_phase);

        const uint32_t ab0 = ab_slot;
        const uint32_t ab_bar0 = smbase + (ab0 << 3);
        mbar_wait_relaxed(ab_bar0, ab_phase);
        advance_ring<AB_RING_SLOTS>(ab_slot, ab_phase);
        mbar_wait_gated_relaxed(smbase + ACC_FREE_OFF,
                                acc_wait_parity, !need_acc_wait);
        mma_cell(el, d_tmem,
                 ab_desc_lo(smbase, A_OFF, ab0, 0), a_desc_hi,
                 ab_desc_lo(smbase, B_OFF, ab0, 0), b_desc_hi,
                 idesc_s0, sfa_t0, sfb_t0, acc_pred);
        mma_cell(el, d_tmem,
                 ab_desc_lo(smbase, A_OFF, ab0, 1), a_desc_hi,
                 ab_desc_lo(smbase, B_OFF, ab0, 1), b_desc_hi,
                 idesc_s1, sfa_t1, sfb_t2, 0xFFFFFFFFu);

        const uint32_t sf1 = sf_slot;
        const uint32_t sf_bar1 = smbase + (sf1 << 3) + SF_READY_OFF;
        mbar_wait_relaxed(sf_bar1, sf_phase);
        cp_sf_buffer(el, sf1, sfa_t0, sfa_t1,
                     sfb_t0, sfb_t1, sfb_t2, sfb_t3,
                     sfa_src_hi, sfa_src_lo, sfb_src_hi, sfb_src_lo);
        commit_consumed(el, smbase,
                        (sf_bar1 + SF_FREE_DELTA) - smbase, commit_mask);
        advance_ring<SF_RING_SLOTS>(sf_slot, sf_phase);
        mma_cell(el, d_tmem,
                 ab_desc_lo(smbase, A_OFF, ab0, 2), a_desc_hi,
                 ab_desc_lo(smbase, B_OFF, ab0, 2), b_desc_hi,
                 idesc_s0, sfa_t0, sfb_t0, 0xFFFFFFFFu);
        mma_cell(el, d_tmem,
                 ab_desc_lo(smbase, A_OFF, ab0, 3), a_desc_hi,
                 ab_desc_lo(smbase, B_OFF, ab0, 3), b_desc_hi,
                 idesc_s1, sfa_t1, sfb_t2, 0xFFFFFFFFu);
        commit_consumed(el, smbase,
                        (ab_bar0 + AB_FREE_DELTA) - smbase, commit_mask);

        const uint32_t sf2 = sf_slot;
        const uint32_t sf_bar2 = smbase + (sf2 << 3) + SF_READY_OFF;
        mbar_wait_relaxed(sf_bar2, sf_phase);
        cp_sf_buffer(el, sf2, sfa_t0, sfa_t1,
                     sfb_t0, sfb_t1, sfb_t2, sfb_t3,
                     sfa_src_hi, sfa_src_lo, sfb_src_hi, sfb_src_lo);
        commit_consumed(el, smbase,
                        (sf_bar2 + SF_FREE_DELTA) - smbase, commit_mask);
        advance_ring<SF_RING_SLOTS>(sf_slot, sf_phase);

        const uint32_t ab1 = ab_slot;
        const uint32_t ab_bar1 = smbase + (ab1 << 3);
        mbar_wait_relaxed(ab_bar1, ab_phase);
        advance_ring<AB_RING_SLOTS>(ab_slot, ab_phase);
        mma_cell(el, d_tmem,
                 ab_desc_lo(smbase, A_OFF, ab1, 0), a_desc_hi,
                 ab_desc_lo(smbase, B_OFF, ab1, 0), b_desc_hi,
                 idesc_s0, sfa_t0, sfb_t0, 0xFFFFFFFFu);
        mma_cell(el, d_tmem,
                 ab_desc_lo(smbase, A_OFF, ab1, 1), a_desc_hi,
                 ab_desc_lo(smbase, B_OFF, ab1, 1), b_desc_hi,
                 idesc_s1, sfa_t1, sfb_t2, 0xFFFFFFFFu);

        const uint32_t sf3 = sf_slot;
        const uint32_t sf_bar3 = smbase + (sf3 << 3) + SF_READY_OFF;
        mbar_wait_relaxed(sf_bar3, sf_phase);
        cp_sf_buffer(el, sf3, sfa_t0, sfa_t1,
                     sfb_t0, sfb_t1, sfb_t2, sfb_t3,
                     sfa_src_hi, sfa_src_lo, sfb_src_hi, sfb_src_lo);
        commit_consumed(el, smbase,
                        (sf_bar3 + SF_FREE_DELTA) - smbase, commit_mask);
        advance_ring<SF_RING_SLOTS>(sf_slot, sf_phase);
        mma_cell(el, d_tmem,
                 ab_desc_lo(smbase, A_OFF, ab1, 2), a_desc_hi,
                 ab_desc_lo(smbase, B_OFF, ab1, 2), b_desc_hi,
                 idesc_s0, sfa_t0, sfb_t0, 0xFFFFFFFFu);
        mma_cell(el, d_tmem,
                 ab_desc_lo(smbase, A_OFF, ab1, 3), a_desc_hi,
                 ab_desc_lo(smbase, B_OFF, ab1, 3), b_desc_hi,
                 idesc_s1, sfa_t1, sfb_t2, 0xFFFFFFFFu);
        commit_consumed(el, smbase,
                        (ab_bar1 + AB_FREE_DELTA) - smbase, commit_mask);

        acc_pred = 0xFFFFFFFFu;
        need_acc_wait = false;
    }
...
void nvfp4_gemm_kernel(const __grid_constant__ CUtensorMap A_tmap,
    ...
    // TMA zero fill supplies zeros in the final partial K512 group.
    const int num_groups = (K + 511) / 512;
    ...
    // Epilogue warp 0 allocates TMEM for the pair. Warps 0-4
    ...
    if (wg == 1) {
        ...
        if (pw == 1 && ptx::lane_id() == 0) {
            ...
            for (int t = cluster_id; t < total_tiles; t += num_clusters) {
                ...
                // [0,128) and B cols [0,128). The odd CTA holds the other halves.
                const int m_blk = m_row * CTA_GROUP + int(m_pair);
                const int n_blk = n_grp * CTA_GROUP + int(m_pair);
                #pragma unroll 1
                for (int g = 0; g < num_groups; ++g) {
                    // Two windows cover eight packed K64 slices.
                    #pragma unroll
                    for (int sub = 0; sub < AB_TMA_SUBS; ++sub) {
                        ...
                        // The tensor map zero-fills past logical K.
                        const CUtensorMap* a_map = &A_tmap;
                        const CUtensorMap* b_map = &B_tmap;
                        const int x = g * 256 + sub * AB_LOAD_BYTES;
            ...
            // own M half). SFB is multicast because the
            ...
            for (int t = cluster_id; t < total_tiles; t += num_clusters) {
                ...
                const int m_blk = m_row * CTA_GROUP + int(m_pair);
                for (int g = 0; g < num_groups; ++g) {
                    // Four SF slots cover two K64 steps each.
                    #pragma unroll
                    for (int sub = 0; sub < 4; ++sub) {
                        ...
                        // Two adjacent VEC16 groups cover one K128 pair.
                        const int sf_group = (g * 4 + sub) * 2;
                ...
                if (aligned32 && (col_base & 15) == 0) {
                    ...
                    for (int q = 0; q < 4; ++q) {
                        ...
                        const bool aligned16 =
                                (reinterpret_cast<uintptr_t>(cdst) & 15u) == 0u;
                        if (aligned16 && c0 + 8 <= N) {
    ...
    // Only epilogue warps rendezvous per CTA. Warp 0 then runs
...
inline CUtensorMap make_sf_tmap(uint8_t* ptr, int outer, int K) {
    ...
    return tmap::encode_tiled_4d_box(
            ...
            /*box_tiles=*/2, /*box_rows=*/4, /*box_cols=*/128,
