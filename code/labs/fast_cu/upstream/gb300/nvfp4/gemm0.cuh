#pragma once
// r0: K192 baseline with threadIdx lane checks.
...
constexpr int AB_LOAD_BYTES = MMA_K;
constexpr int AB_TMA_SUBS   = 4;
static_assert(AB_LOAD_BYTES == 96,
              "K=192 packed-FP4 stage must be 96 bytes wide");
...
static_assert(AB_TX == 49152u && SF_TX == 9216u,
...
//
// The baseline stages four K=192 slices. Each slot supplies two K=96 MMAs at
// packed-byte offsets 0 and 48, both inside the same swizzle window.
// ============================================================================
namespace nvfp4 {
...
static __device__ __forceinline__ MainloopState mainloop_tile(
    ...
    if (full_groups >= 1) {
        const uint32_t num_stages = full_groups * 4u;
        uint32_t stage = 0;
        do {
            // One K=192 scale slot and one K=192 A/B slot feed two K=96
            // MMAs. Return each SMEM source through an ordered commit as soon
            // as its final asynchronous reader has been issued.
            const uint32_t sf_bar = smbase + (sf_slot << 3) + SF_READY_OFF;
            mbar_wait_relaxed(sf_bar, sf_phase);
            cp_sf_buffer(el, sf_slot, sfa_t0, sfa_t1, sfa_t2,
                         sfb_t0, sfb_t1, sfb_t2, sfb_t3, sfb_t4, sfb_t5,
                         sfa_src_hi, sfa_src_lo, sfb_src_hi, sfb_src_lo);
            commit_consumed(el, smbase, (sf_bar + SF_FREE_DELTA) - smbase,
                            commit_mask);

            const uint32_t ab_bar = smbase + (ab_slot << 3);
            mbar_wait_relaxed(ab_bar, ab_phase);
            ...
            commit_consumed(el, smbase, (ab_bar + AB_FREE_DELTA) - smbase,
                            commit_mask);
            need_acc_wait = false;

            const uint32_t next_sf = sf_slot + 1;
            const bool wrap_sf = (next_sf == SF_RING_SLOTS);
            sf_slot = wrap_sf ? 0u : next_sf;
            sf_phase ^= wrap_sf ? 1u : 0u;

            const uint32_t next_ab = ab_slot + 1;
            const bool wrap_ab = (next_ab == AB_RING_SLOTS);
            ab_slot = wrap_ab ? 0u : next_ab;
            ab_phase ^= wrap_ab ? 1u : 0u;
        ...
        if (pw == 1 && (threadIdx.x % 32) == 0) {
            ...
            for (int t = cluster_id; t < total_tiles; t += num_clusters) {
                ...
                for (int g = 0; g < num_groups; ++g) {
                    // Four 96-byte loads cover the packed K=768 group.
                    #pragma unroll
