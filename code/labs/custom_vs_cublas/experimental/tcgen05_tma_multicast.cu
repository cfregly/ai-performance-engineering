/**
 * TMA Multicast with SM90_TMA_LOAD_MULTICAST
 * ==========================================
 * 
 ...
 * - Multicast mask determines which CTAs receive the data
 * - Reduces memory bandwidth by 2x for B tile loads
 * 
...
struct TmaMulticastSharedStorage {
...
// Use TMA multicast for B tiles
using TmaLoadA = SM90_TMA_LOAD;
using TmaLoadB = SM90_TMA_LOAD_MULTICAST;
...
gemm_tma_multicast(ATensor mA, BTensor mB, CTensor mC, DTensor mD,
...
torch::Tensor run_tma_multicast_matmul(torch::Tensor a, torch::Tensor b) {
  ...
  using SharedStorageT = TmaMulticastSharedStorage<TypeA, TypeB, decltype(sA_layout), decltype(sB_layout)>;
  ...
  auto* kernel_ptr = &gemm_tma_multicast<
...
torch::Tensor matmul_tcgen05_tma_multicast(torch::Tensor a, torch::Tensor b) {
  return tma_multicast_impl::run_tma_multicast_matmul(a, b);
}
