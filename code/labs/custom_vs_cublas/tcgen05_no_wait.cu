/**
 * Stage 15: No MMA Wait - CUTLASS Producer/Consumer Pattern
 * ==========================================================
 * 
 * CUTLASS pattern with TWO barrier arrays:
 * - full_barrier: Producer (TMA) signals when data is ready
 * - empty_barrier: Consumer (MMA) signals when stage is free (via umma_arrive)
 * 
 * Key insight: Consumer does NOT wait inside the mainloop!
 * - Consumer just signals empty_barrier via umma_arrive
 * - Producer waits on empty_barrier BEFORE issuing TMA (if needed)
 * - This allows compute and load to overlap
 * 
 * The only explicit wait before epilogue is on a dedicated mma_barrier.
 */
...
struct NoWaitSharedStorage {
  ...
  alignas(16) cute::uint64_t full_barrier[kStages];   // TMA signals when data ready
  alignas(16) cute::uint64_t empty_barrier[kStages];  // MMA signals when data consumed
  alignas(16) cute::uint64_t mma_barrier;             // For epilogue sync
...
gemm_no_wait(ATensor mA, BTensor mB, CTensor mC, DTensor mD,
  ...
  // Prologue: Fill first (kStages-1) stages
  if (elect_one_warp && elect_one_thr) {
...
torch::Tensor run_no_wait_matmul(torch::Tensor a, torch::Tensor b) {
  ...
  using SharedStorageT = NoWaitSharedStorage<TypeA, TypeB, decltype(sA_layout), decltype(sB_layout)>;
...
torch::Tensor matmul_tcgen05_no_wait(torch::Tensor a, torch::Tensor b) {
  return no_wait_impl::run_no_wait_matmul(a, b);
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("matmul_tcgen05_no_wait", &matmul_tcgen05_no_wait);
}
