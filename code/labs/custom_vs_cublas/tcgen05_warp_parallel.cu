/**
 * True Warp Specialization - Producer and Consumer in Parallel
 * =============================================================
 * 
 * Simplified version:
 * - Thread 0 (one thread!) does TMA (producer)
 * - Thread 0 (same thread) also does MMA (consumer) 
 * - But they overlap: issue TMA, then MMA, then wait for TMA
 * 
 * The key insight is that TMA is ASYNC and doesn't need a dedicated warp.
 * One thread can issue TMA, start MMA, and the hardware runs them in parallel.
 */
...
struct WarpParallelSharedStorage {
...
gemm_warp_parallel(ATensor mA, BTensor mB, CTensor mC, DTensor mD,
  ...
  // =========================================================================
  // KEY OPTIMIZATION: Issue TMA then MMA then wait
  // This allows TMA and MMA to overlap since TMA is async
  // =========================================================================
...
torch::Tensor run_warp_parallel_matmul(torch::Tensor a, torch::Tensor b) {
  ...
  using SharedStorageT = WarpParallelSharedStorage<TypeA, TypeB, decltype(sA_layout), decltype(sB_layout)>;
  ...
  auto* kernel_ptr = &gemm_warp_parallel<
...
torch::Tensor matmul_tcgen05_warp_parallel(torch::Tensor a, torch::Tensor b) {
  return warp_parallel_impl::run_warp_parallel_matmul(a, b);
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("matmul_tcgen05_warp_parallel", &matmul_tcgen05_warp_parallel);
}
